...
class Rocket(pygame.sprite.Sprite):
	def __init__(self, winsize):
		...
		self.shield = False
		self.double_shot = False #Update 1
		self.shield_timer = 0
	...
	def handle_powerups(self):
		if self.shield:
			self.shield_timer += 1
			if self.shield_timer > 300:  # Shield lasts for 10 seconds
				self.shield = False
				self.shield_timer = 0

		if self.double_shot:
			self.double_shot_timer += 1
			if self.double_shot_timer > 300:  # Double shot lasts for 10 seconds
				self.double_shot = False
				self.double_shot_timer = 0

	def toggle_double_shot(self): #Update 1
		self.double_shot = not self.double_shot

class Bullet(pygame.sprite.Sprite):
	def __init__(self, pos, dir_, winsize, offset=0): #Update 2
		super(Bullet,self).__init__()

		self.pos = pos
		self.dir = dir_
		self.winwidth, self.winheight = winsize
		self.offset = offset #Update 2
	...
	def get_bullet_pos(self, pos): #Update 3
		if self.dir == 'top':
			pos = (pos[0]+32+self.offset, pos[1]-10)
		elif self.dir == 'left':
			pos = (pos[0]-10, pos[1]+32+self.offset)
		elif self.dir == 'right':
			pos = (pos[0]+74, pos[1]+32+self.offset)
		elif self.dir == 'bottom':
			pos = (pos[0]+32+self.offset, pos[1]+74)
